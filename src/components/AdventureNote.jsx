...
export default function AdventureNote({ currentNote, getAdventureNotes }) {
  ...
  const deleteAdventureNote = () => {
    axios
      .delete(`http://localhost:8080/user/adventure_note/${currentNote.id}`, {
        headers: {
    ...
    <div className="adventure-note-card">
      ...
      <div className="buttons-container-note">
        <AiIcons.AiOutlineEdit
          ...
          onClick={() =>
            navigate("/dashboard/edit-adventure-note", {
              state: { adventureNote: currentNote },
            })
          }
        />
        <AiIcons.AiOutlineDelete
          className="delete-note-button"
          onClick={deleteAdventureNote}
        />
      </div>
    </div>
  );
